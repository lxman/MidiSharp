...
public sealed class EditorWindow : IDisposable
{
    ...
    {
        if (gui is not { HasEditor: true } || !gui.IsApiSupported("x11", floating: false)) return null;
    ...
    private void Run()
    {
        try
        {
            ...
            // Embed the editor: create (embedded) → scale → parent into our window → show.
            if (!_gui.Create("x11", floating: false)) { _error = "plugin gui create(x11) failed."; Teardown(); _ready.Set(); return; }
            _gui.SetScale(1.0);
            if (!_gui.SetParent("x11", _window)) { _error = "plugin gui set_parent failed."; _gui.Destroy(); Teardown(); _ready.Set(); return; }
            _gui.Show();
