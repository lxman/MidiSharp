...
// A plugin's editor hosts the plugin's own GUI in this process. On Windows that GUI is often COM-based — a
// VST3 IPlugView builds Direct2D/WIC/OLE objects in createView — and those calls fault (0xC0000005) on an MTA
// thread. But the .NET runtime makes the process main thread MTA, and a thread's apartment can't be changed
// once set. So run the whole worker on a dedicated STA thread on Windows: one thread loads the plugin,
// processes audio, AND shows the editor, which also satisfies CLAP's rule that the editor runs on the
// plugin's creation thread. Apartments don't exist on Linux/macOS (and SetApartmentState throws there), so
// run inline — the X11/Cocoa editor paths don't need this.
if (OperatingSystem.IsWindows())
{
    var rc = 0;
    var worker = new Thread(() => rc = RunWorker(args)) { Name = "midisharp-worker" };
...
{
    // STA is established above; initialize the OLE layer (clipboard/drag-drop) that some editors rely on.
    if (OperatingSystem.IsWindows()) WorkerNative.OleInitialize(IntPtr.Zero);
