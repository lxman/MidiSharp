...
internal static class Inspect
{
    public static void DumpChunks(string path, int maxChunks = 50)
    {
        ...
        for (var c = 0; c < maxChunks && pos < data.Length - 8; c++)
        {
            ...
            string? form = (tag == "LIST" && pos + 12 <= data.Length) ? Encoding.ASCII.GetString(data, pos + 8, 4) : null;
            var note = "";
            ...
            {
                int subPos = pos + 12;
                int subEnd = pos + 8 + (int)size;
                while (subPos + 8 <= Math.Min(subEnd, data.Length))
                {
                    string subTag = Encoding.ASCII.GetString(data, subPos, 4);
