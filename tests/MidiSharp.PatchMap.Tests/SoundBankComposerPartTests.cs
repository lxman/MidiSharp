...
public class SoundBankComposerPartTests
{
    ...
    public void PartOverride_PlacedAtSyntheticAddress_AndMapped()
    {
        ...
        using IRBank flute = TestBanks.OneSamplePatch("flute", 0.6f, 0, 73, "Flute");
        var partOverrides = new Dictionary<(int, int), PatchRef> { [(0, 5)] = new PatchRef(flute, 0, 73) };

        CompositeResult result = SoundBankComposer.BuildComposite(baseBank, NoPatchOverrides, NoTrackOverrides, partOverrides);

        int key = PartKey(0, 5);
        Assert.True(result.PartPatchMap.TryGetValue(key, out (int Bank, int Program) addr));
    ...
    public void TwoChannels_SameTrack_SubstituteIndependently()
    {
        ...
        using IRBank baseBank = TestBanks.OneSamplePatch("base", 0.1f, 0, 0, "Piano");
        using IRBank flute = TestBanks.OneSamplePatch("flute", 0.6f, 0, 73, "Flute");
        using IRBank bass = TestBanks.OneSamplePatch("bass", 0.4f, 0, 33, "Bass");
        ...
        Patch? ch5 = result.Bank.FindPatch(result.PartPatchMap[PartKey(0, 5)].Bank, result.PartPatchMap[PartKey(0, 5)].Program);
        Patch? ch6 = result.Bank.FindPatch(result.PartPatchMap[PartKey(0, 6)].Bank, result.PartPatchMap[PartKey(0, 6)].Program);
    ...
    public void PartAndTrackOverride_DoNotCollide_OnDistinctBanks()
    {
        ...
        using IRBank baseBank = TestBanks.OneSamplePatch("base", 0.1f, 0, 0, "Piano");
        using IRBank strings = TestBanks.OneSamplePatch("strings", 0.5f, 0, 48, "Strings");
        using IRBank flute = TestBanks.OneSamplePatch("flute", 0.6f, 0, 73, "Flute");
        var trackOverrides = new Dictionary<int, PatchRef> { [0] = new PatchRef(strings, 0, 48) };
        var partOverrides = new Dictionary<(int, int), PatchRef> { [(0, 5)] = new PatchRef(flute, 0, 73) };

        CompositeResult result = SoundBankComposer.BuildComposite(baseBank, NoPatchOverrides, trackOverrides, partOverrides);

        (int Bank, int Program) trackAddr = result.TrackPatchMap[0];
        (int Bank, int Program) partAddr = result.PartPatchMap[PartKey(0, 5)];
