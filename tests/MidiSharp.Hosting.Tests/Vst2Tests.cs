...
public sealed class Vst2Tests
{
    ...
    {
        PluginDescriptor? d = _format.Scan(_format.DefaultSearchPaths).FirstOrDefault(p => p.Name == "MidiSharp VST2 Gain");
        return d == null ? null : _format.Load(d, Config);
    }

    [Fact]
    public void Reports_its_editor_and_size_through_eff_edit_get_rect()
    {
        ...
        Assert.NotNull(gui);
        Assert.True(gui!.HasEditor, "the fixture sets effFlagsHasEditor.");
        Assert.True(gui.IsApiSupported("x11", floating: false));
    ...
    [Fact]
    public void Embeds_its_editor_in_a_native_window()
    {
        ...
        using EditorWindow? window = MidiSharp.Hosting.EditorHost.EditorWindow.Open(plugin!.Gui, "VST2 editor test");
    ...
    public void Midi_cc_event_applies_sample_accurately()
    {
        ...
        const int off = 300;
        float[] l = RunBlock(e => e.QueueEvent(HostEvent.Midi(off, 0xB0, 7, 127)));
