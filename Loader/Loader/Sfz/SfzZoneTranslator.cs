...
internal static class SfzZoneTranslator
{
    ...
    private static List<ModulationRoute> BuildRoutes(SfzRegion r, bool suppressVelocityRoute = false)
    {
        ...
        if (veltrack != 0)
        {
            routes.Add(new ModulationRoute
            {
                Source = new ModSource.Velocity(),
                Dest = ModDestination.AttenuationDb,
                Amount = Math.Abs(veltrack) / 100.0 * 40.0,
                Transform = veltrack > 0 ? ModTransform.ConcaveUnipolarNegative : ModTransform.ConcaveUnipolar,
            });
        }
